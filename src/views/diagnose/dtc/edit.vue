<!-- Discription: 故障码编辑, author: ydj, Date: 2019-11-09 09:52:23 -->
<template>
  <div>
    <el-row>
      <el-card class="box-card">
        <div slot="header" class="clearfix">
          <span>{{ sn ? '编辑故障码' : '添加故障码' }}</span>
        </div>
        <div>
          <el-form ref="postForm" :model="form" class="form-container">
            <el-row>
              ...
              <el-row>
                <el-col :span="24">
                  <el-form-item label-width="10%" label="部 位" class="postInfo-container-item">
                    <el-input
                      v-model="form.position"
                      placeholder="请输入故障码发生部位"
                      style="width:88%;max-width:100%;"
                    />
                  </el-form-item>
                </el-col>
...
<script>
import { querDtcBySN, createDtc, updateDtc } from '@/api/diagnose/dtc'
import {
  percentStageOptions,
  eventStatusOptions
} from '@/api/contacts/options'
import { DashSplit } from '@/components/split/index.js'
...
export default {
  ...
  data() {
    return {
      ...
      percentOptions: percentStageOptions,
      eventOptions: eventStatusOptions
    }
  },
  created() {
  ...
  methods: {
    handleSave(sn) {
      if (sn) {
        ...
        updateDtc(sn, this.form)
          .then(response => {
            if (response.data.status === 80200) {
              this.$message({ message: '保存成功', type: 'success' })
              this.$router.replace({ path: '/diagnose/dtc/list' })
            }
          })
          .catch(error => {
            console.log(error)
