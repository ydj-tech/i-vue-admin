<!-- Discription: 记录, author: ydj, Date: 2019-12-08 18:52:47 -->
<template>
  <div>
    <el-row>
      <el-card class="box-card">
        ...
        <div style="margin-bottom:50px;">
            ...
            <el-form :inline="true" class="demo-form-inline">
              <el-form-item label>
                <el-select v-model="type" placeholder="故障码/数据流/版本信息">
                  <el-option label="故障码" value="dtc" />
                  <el-option label="数据流" value="cds" />
                  <el-option label="版本信息" value="ver" />
                  <el-option label="登陆信息" value="login" />
                </el-select>
              </el-form-item>
              <el-form-item>
          ...
          >
            <el-table-column type="expand">
              <template slot-scope="props">
                <el-form label-position="left" class="demo-table-expand">
                  <template v-if="event === 'login' ? true : false">
                  ...
                  <template v-if="event === 'ver' ? true : false">
                    <el-table
                      :data="props.row.con"
                      stripe
                      fit
                  ...
                  <template v-if="event === 'cds' ? true : false">
                    <el-table
                      :data="props.row.con"
                      stripe
                      fit
            ...
            <el-table-column v-if="event === 'login' ? true : false" label="昵称" align="center">
              <template slot-scope="scope">
                <span>{{ scope.row.con.nickname }}</span>
              </template>
            </el-table-column>
            <el-table-column v-if="event === 'login' ? true : false" label="头像" align="center">
              <template slot-scope="scope">
                <div>
...
import { pageList } from "@/api/diagnose/record";
import {
  eventStatusOptions,
  percentStageOptions
...
export default {
  ...
  methods: {
    async init() {
      try {
        const response = await pageList(this.listQuery, this.type);
